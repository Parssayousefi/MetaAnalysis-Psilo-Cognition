...
```{r setup,results='hide'}
 knitr::opts_knit$set(root.dir = "C:/Users/prsyu/OneDrive/Bidlung/University/M.S. Leiden University/M.S. Neuroscience (Research)/Internship Groningen/GitHub_Multiverse_meta/MetaAnalysis-Psilo-Cognition")
...
# calculate_meta(RT=TRUE)$model_uni
```
...
```{r Figure S7,  fig.show='hold', fig.align='center'}
...
```{r forest plot bauastelle}	
  ...
  # Create a data frame for the forest plot
  forest_data <- data.frame(
    Study = plot_data$PlotLabel,
    Dose = plot_data$DoseLabel,
    CognitiveFunction = plot_data$CognitiveFunction,
    N = plot_data$N_final,
    Timepoint = plot_data$Timepoint,
    ES = plot_data$yi,
  ...
  # Order the table according to the Dose column
  forest_data <- forest_data %>%
    mutate(Dose = factor(Dose, levels = c("Micro", "Low", "Medium", "High"))) %>%
    arrange(Dose)
    print(forest_data)
  ...
  # Create the forest plot
  p <- forestploter::forest(
    forest_data[,c("Study", "Dose", "CognitiveFunction", "N", "Timepoint", " ", "CI")],
    est = forest_data$ES,
    lower = forest_data$LowerCI,
    upper = forest_data$UpperCI,
    ci_column = 5,
...
```{r Figure S1 Convergence graph Metaforest RT,cache = TRUE}
...
```{r Figure S3 Variable/Permutation Importance RT, cache = TRUE}
