...
Read_Prepare <- function(datafile) {
    ...
    cols_to_select <- c("ID", "CognitiveFunction", "N_final", "ES_final_RT", "ES_final_ACC", 
                        "ES_final_other", "Timepoint (min)", "PlotLabel", "DoseLabel", "EF sensitivity (1-3)")
    ...
    if (!is.null(data_combined) && nrow(data_combined) == nrow(data)) {
        ...
        data_combined$dosage_cat_micro <- c(rep(1, nrow(data_micro)),
                                            rep(0, nrow(data_low) + nrow(data_mid) + nrow(data_high)))
...
```{r}
# Count the number of rows where at least one of the specified columns is not NA
#num_rows <- nrow(data[!is.na(data$ES_final_RT) | !is.na(data$ES_final_ACC), ])
#print(data[is.na(data$ES_final_RT) | is.na(data$ES_final_ACC), ])

# Print the number of rows
#print(paste("Number of rows where 'ES_final_RT' or 'ES_final_ACC' is not NA:", num_rows))

num_rows <- nrow(data[is.na(data$ES_final_RT), ])
print(num_rows)
```
...
```{r}
# Print rows with EF final RT > 2.5
# high_ef_rt_rows <- data_RT[data$ES_final_RT > 2.5, ]
# print(high_ef_rt_rows)
calculate_meta(RT=TRUE, plot = TRUE,  multilevel=TRUE)
...
identify_outliers <- function(data_type, multi=FALSE) {
  ...
  # Assuming calculate_meta is a predefined function that you have access to
  model_results <- calculate_meta(RT=data_type == "RT", ACC=data_type == "ACC", multilevel=multi)
  ...
  # Assuming rma.mv is from the 'metafor' package and you have previously defined and loaded it
  filtered_data <- data[!data$outlier, ]
  model_no_outlier <- rma.mv(yi = filtered_data[[effect_size_col]], 
                             V = filtered_data[[variance_col]],
                             random = ~ 1 | ID/ES_ID, 
...
# Note: Ensure that the column names "ID", "PlotLabel", "DoseLabel", "Cognitive measure", and "Timepoint (min)"
# exactly match those in your data_RT and data_ACC datasets.
# Also, ensure that the `calculate_meta` and `rma.mv` functions are defined and accessible in your environment.
...
calculate_residuals(ACC= TRUE, outlier_threshold = 3)
...
moderation_analysis( RT = TRUE, moderation_var = "dosage_cat_3")#low vs mid vs high
moderation_analysis( RT = TRUE, moderation_var = "dosage_cat_2")#low vs mid & high
