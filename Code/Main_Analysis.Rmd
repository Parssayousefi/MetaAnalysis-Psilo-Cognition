...
```{r TIming: pre vs during vs post peak}
...
#ClassifyTiming(data, start_peak=120,stop_peak = 180)
...
```{r wide  peak window}	
data_RT <- ClassifyTiming(data_RT, start_peak=60,stop_peak = 210)
nrow(data_RT)
length(unique(data_RT$ID))
table(data_RT$timing)
#moderation_analysis( RT = TRUE, moderation_var = "timing")
...
model <- rma.mv(yi = data$ES_final_RT, 
                ...
                data = data,
                method = "REML",
                mods = as.formula("~ `EF_sensitivity`"))
summary(model)
```
