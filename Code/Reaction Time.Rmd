...
```{r Forest plot Function "working here"}
...
funnel_plot <- function(RT = FALSE, ACC = FALSE) {
    ...
    if (RT) {
        ...
        xlim_values <- c(-1, 3)
        ylim_values <- c(0.6, 0)
        color_points <- brewer.pal(nrow(unique(plot_data)), "Dark2")
    } else if (ACC) {
        model_results <- calculate_meta(ACC = TRUE)
        forest_title <- "Forest plot for the multilevel model of Accuracy"
        funnel_title <- "Funnel plot for the multilevel model of Accuracy"
        ...
        xlim_values <- c(-2.5, 2)
        ylim_values <- c(0.9, 0)
        color_points <- brewer.pal(nrow(unique(plot_data)), "Paired")
    } else {
        stop("Either RT or ACC must be set to TRUE.")
    }
    ...
    # Reverse the palette if needed to match the order of light to dark for contour shades
    #cantour_shades <- rev(brewer_palette)  # Use rev() if you need the darkest color for the highest contour
    ...
    # Add the points using the points function
    points(point_data$x, point_data$y, col = point_data$col, pch = 16, cex = 2)
...
install.packages("viewports")

funnel_plot_hybrid <- function(RT = FALSE, ACC = FALSE) {
    # Fetch the model results using calculate_meta
    if (RT) {
        model_results <- calculate_meta(RT = TRUE)
        plot_data <- data_RT
        effect_size_label <- "Reaction Time (Hedges' g)"
        xlab_text <- "Reaction Time (Hedges' g)"
        xlim_values <- c(-1, 3)
        ylim_values <- c(0, 0.6)
        color_palette <- brewer.pal(nrow(unique(plot_data$PlotLabel)), "Dark2")
    } else if (ACC) {
        model_results <- calculate_meta(ACC = TRUE)
        plot_data <- data_ACC
        effect_size_label <- "Accuracy (Hedges' g)"
        xlab_text <- "Accuracy (Hedges' g)"
        xlim_values <- c(-2.5, 2)
        ylim_values <- c(0, 0.9)
        color_palette <- brewer.pal(nrow(unique(plot_data)), "Paired")
    } else {
        stop("Either RT or ACC must be set to TRUE.")
    }
  
    # Prepare plot data
    plot_data$yi <- ifelse(RT, plot_data$ES_final_RT, plot_data$ES_final_ACC)
    plot_data$sei <- sqrt(ifelse(RT, plot_data$ES_var_RT, plot_data$ES_var_ACC))
    plot_data$color <- as.factor(plot_data$PlotLabel)

    # Prepare colors
    color_palette <- setNames(brewer.pal(n_distinct(plot_data$color), "Set1"), levels(plot_data$color))
    color_points
    # Setup ggplot
    p <- ggplot(plot_data, aes(x = yi, y = sei, color = color)) +
        geom_point(shape = 16) +
        scale_color_manual(values = color_palette) +
        labs(x = effect_size_label, y = "Standard Error", title = "Funnel Plot with Contours") +
        theme_minimal() +
        theme(legend.title = element_blank()) +
        xlim(xlim_values) + ylim(ylim_values)  # Setting axis limits based on the provided values

    # Print initial ggplot
    print(p)

    # Capture the ggplot as a grob and plot it on a viewport
    grid.newpage()
    g <- ggplotGrob(p)
    grid.draw(g)

    # Since ggplot2 does not allow adding metafor plots directly, we recreate this plot using base plotting in a new viewport
    pushViewport(viewport(width = 0.8, height = 0.8))
    par(new = TRUE, mar = c(5.1, 4.1, 4.1, 2.1))

    # Add metafor funnel plot lines and trim-and-fill
    contour_shades <- brewer.pal(9, "Blues")[c(2, 4, 6)]
    funnel(model_results$model_multi,
        refline = model_results$model_uni$beta,
        xlim = xlim_values,
        ylim = ylim_values,
        xlab = xlab_text,
        ylab = "Standard Error",
    ...
    trim <- trimfill(model_results$model_uni, add = TRUE, col.trim = "red")

    # Return control to the top-level viewport
    popViewport()
}

# Example call
funnel_plot_hybrid(ACC = TRUE)
