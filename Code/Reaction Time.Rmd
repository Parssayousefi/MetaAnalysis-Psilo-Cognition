...
funnel_plot <- function(RT = FALSE, ACC = FALSE) {
    ...
    color_mapping <- setNames(hcl.colors(length(unique(plot_data$PlotLabel)), "Green-Brown"), unique(plot_data$PlotLabel))
    model <- model_results$model_multi
    ...
    # Create a contour-enhanced funnel plot
    funnel(model,
        refline = model_uni$beta,
        refline2=0,
        lty = c(0,5),
        level = c(90, 95, 99), # Adding multiple levels for contours
        shade = c("gray90", "gray55", "gray75"), 
        label = "all", # Label all points with study labels
        slab = plot_data$ES_ID,
        xlab=  "Reaction Time (Hedges' g)",
        back = "white",
        cex= 2,
        lwd = 3,
        xlim = c(-1, 3.5),
    ...
    # Add filled studies
    points(filled_studies, filled_sei, col = "red", pch = 16, cex = 2)
    ...
    legend("topright",ncol = 1,
        ...
        fill = c("gray75", "gray90", "gray55","red", color_mapping[legend_data$PlotLabel]),
