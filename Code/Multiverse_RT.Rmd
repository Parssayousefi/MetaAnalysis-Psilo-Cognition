...
calculate_meta(RT=TRUE, multilevel=TRUE)$ci.ub
```
...
calculate_meta_no_outliers <- function(RT=FALSE, ACC=FALSE, multilevel=TRUE) {
    ...
    if(multilevel) {
        ...
        overall_ci_lb <- model$ci.lb
        overall_ci_ub <- model$ci.ub
        ...
        print("Model results with outliers:")
        print(summary(model))
        print("Model results No outlier:")
        print(summary(model_no_outlier))

        print("model comparison nested vs non-nested:")
        print(anova(model,model_no_outlier))
...
```
```{r}
calculate_meta_no_outliers(RT = TRUE, multilevel = TRUE)
