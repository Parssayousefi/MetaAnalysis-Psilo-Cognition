...
calculate_meta <- function(RT=FALSE, ACC=FALSE, plot=FALSE, multilevel=FALSE) {
...
```{r}
calculate_meta(RT=TRUE, multilevel=TRUE)$b
```
```{r }
...
identify_outliers <- function(data_type, multi=FALSE) {
  ...
  if(data_type == "RT") {
    data <- as.data.frame(data_RT)  # Convert to dataframe
    effect_size_col <- "ES_final_RT"
    variance_col <- "ES_var_RT"
    print("Analyzing outliers for Reaction Times")
  } else if (data_type == "ACC") {
    data <- as.data.frame(data_ACC)  # Convert to dataframe
    effect_size_col <- "ES_final_ACC"
    variance_col := "ES_var_ACC"
  ...
  # Assume 'calculate_meta' updates 'data' to include 'ci.lb' and 'ci.ub' for overall model results
  overall_ci_lb <- model_results$ci.lb
  overall_ci_ub <- model_results$ci.ub
  
  # Adjust 'd' to match your effect size column
  ...
  # Assuming 'calculate_meta' returns a pooled effect size, which needs to be defined
  # For this example, let's use a simple mean of 'd' as a placeholder
  pooled_effect <- mean(data$d, na.rm = TRUE)
  ...
  plot <- ggplot(data = data, aes(x = d, colour = outlier, fill = outlier)) +
    ...
    theme_bw() +
    labs(title="Effect Size Distribution", x="Effect Size", y="Frequency") +
    scale_fill_manual(values = c("FALSE" = "grey", "TRUE" = "red")) +
    scale_colour_manual(values = c("FALSE" = "black", "TRUE" = "red"))
  print(plot)
  # Print and return outliers
...
identify_outliers("RT", multi = FALSE)
```
```{r}
...
calculate_residuals <- function(RT = FALSE, ACC = FALSE, outlier_threshold = 2) {
    ...
    #display
    library(gridExtra)
    grid_arrangement <- grid.arrange(boxplot, barplot, ncol = 1)
...
mod_all_ml_no_outliers <- rma.mv(yi     = ES_final_RT,
                                 ...
                                 random = ~ 1 | ID, 
                                 data   = data_RT)
print(mod_all_ml_no_outliers$QE)
?rma.mv
```
