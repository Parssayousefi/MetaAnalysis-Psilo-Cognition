...
```{r}
multilevel_meta <- function(data, effect_size_col,variance_col, id_col, print = FALSE) {
  # Ensure column names are in the data
  if (!(effect_size_col %in% names(data)) || !(variance_col %in% names(data)) || !(id_col %in% names(data))) {
    stop("One or more specified columns are not in the data frame.")
  }
  
  # formula for random effects
  random_formula <- eval(parse(text = paste("~ 1 |", id_col)))
  ...
  # print the I^2 statistic
  # Return the model
  if (print) {
  cat("Updated heterogeneity in % (ignore model heterogeneity!):\n", I2, "\n")
}
...
multilevel_meta(data = data_RT,effect_size_col = "ES_final_RT",variance_col = "ES_var_RT",id_col = "ID")
head(data_RT)
```
```{r}
model <- rma.mv(yi = data_RT$ES_final_RT, 
                V = data_RT$ES_var_RT, 
                random = ~ 1 | ID/ES_ID, 
                data = data_RT,
                test = "t",
...
```{r}
#Residuals raw
(res_mod_all_plain <- residuals(multilevel_meta(data = data_RT,effect_size_col = "ES_final_RT",variance_col = "ES_var_RT",id_col = "ID")))  
```

```{r}
#residuals standardized

(res_mod_all_ml    <- rstandard(multilevel_meta(data = data_RT,effect_size_col = "ES_final_RT",variance_col = "ES_var_RT",id_col = "ID")))
```
...
```{r}
zresids <- data.frame(ID = data_RT[,1], Residuals = res_mod_all_ml$z)
