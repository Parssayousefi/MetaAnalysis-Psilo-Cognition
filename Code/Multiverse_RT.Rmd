...
```{r}
plot_residuals <- function(RT = FALSE, ACC = FALSE, plot_type = "barplot") {
    ...
    if (plot_type == "boxplot") {
        # Boxplot code
        data_for_plot <- data.frame(z = res_mod_all_ml$z, ID = data_for_ID$ID)
        ...
        ggplot(data_for_plot, aes(x = "", y = z)) +
            geom_boxplot() +
            geom_text(data = dplyr::filter(data_for_plot, outlier), aes(label = ID), nudge_y = 0.2) +
            theme_gray() +
        ...
        ggplot(data_for_barplot, aes(x = ID, y = Residuals)) +
        ...
        stop("Invalid plot type. Choose either 'barplot' or 'boxplot'.")
    }
    return(invisible(res_mod_all_ml))
}
plot_residuals(ACC= TRUE,plot_type = "boxplot" )
head(data_RT)
```
