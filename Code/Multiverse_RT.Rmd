...
```{r}
multilevel_meta <- function(data, effect_size_col,variance_col, heterogeneity = FALSE) {
  # Ensure column names are in the data
  if (!(effect_size_col %in% names(data)) || !(variance_col %in% names(data))) {
...
multilevel_meta(data = data_RT,effect_size_col = "ES_final_RT",variance_col = "ES_var_RT", heterogeneity =TRUE)
```
```{r}
