...
summary(calculate_meta(ACC=TRUE, multilevel=TRUE)$model_multi)
calculate_meta(ACC=TRUE, multilevel=TRUE)$variance_analysis$ResultsTable
calculate_meta(ACC=TRUE, multilevel=TRUE)$variance_analysis$TotalI2

```

```{r Figure 5  Forrest plot ACC}
library(metafor)
...
# 3.4.2.2 Model comparison nested vs non-nested
```{r}
calculate_meta(ACC=TRUE, multilevel=TRUE)$model_comparison
```

# 3.4.2.3 Subgroup Analyses ACC
...
# 3.4.2.3.1 Timing as moderator ACC
...
# 3.4.2.3.2 Dosage as moderator ACC
```{r Dosage}
...
```
# 3.4.2.3.3 Cognitive function as moderator
...
print(unique(data_ACC$CognitiveFunction))
```
# 3.4.2.3.4 EF sensitivity as moderator
...
# 3.4.2.3.5 Metaforest Analysis ACC
...
run_metaforest_analysis <- function(data_type) {
  ...
  if (data_type == "RT") {
    ...
    metaforest_formula <- ES_final_RT~CognitiveFunction  + EF_sensitivity  + timing + dosage_cat_all
  } else if (data_type == "ACC") {
    data_metaforest <- data_ACC
    outcome_var <- "ES_final_ACC"
    vi_variable <- "ES_var_ACC"
    metaforest_formula <- ES_final_ACC~CognitiveFunction  + EF_sensitivity  + timing + dosage_cat_all
  } else {
...
```{r Figure S10 Convergence graph Metaforest ACC}
...
```
```{r Figure S11 retained moderators ACC}
...
```{r Figure S12 Variable/Permutation Importance ACC}
predictors <- c("EF_sensitivity", "CognitiveFunction", "dosage_cat_all")  # adjust based on previous analysis
...
```{r Figure S13 Partial Dependence Plot ACC}
...
```{r Figure S14 Moderated Partial Dependence Plot ACC}
#moderated partial dependence plot 
PartialDependence(final, moderator = "dosage_cat_all",rawdata = TRUE, pi = .95)


```

# 3.4.2.4 Publication Bias ACC
```{r}
#kendals tau
ranktest(x = data_ACC$ES_final_ACC, vi = data_ACC$ES_var_ACC)
...
```
# 3.4.2.5 outliers & influential cases ACC
...
diagnostic_analysis <- function(data_type, threshold = 2) {
    ...
    print(outliers_dfbetas)
    
    data[outliers_dfbetas, "ES_ID"]

    outliers <- (outliers_cooks, outliers_hatvals, outliers_dfbetas)
    ...
    model_no_outliers <- rma.mv(yi = data_no_outliers[[ifelse(data_type == "RT", "ES_final_RT", "ES_final_ACC")]],
                                #yi = data_no_outliers$ES_final_ACC,
                                #V = data_ACC$ES_var_ACC,
...
```{r}
data_ACC <- data_ACC[data_ACC$ES_ID != 24, ]
nrow(data_ACC)
data_RT <- data_RT[data_RT$ES_ID != 7, ]
data_RT <- data_RT[data_RT$ES_ID != 37, ]
