---
title: "Meta Analysis Psilocybin Cognition: Accuracy"
...
nested model:
```{r }
print(calculate_meta(ACC = TRUE)$model_multi)
print(calculate_meta(ACC = TRUE)$variance_analysis$ResultsTable)
print(calculate_meta(ACC = TRUE)$variance_analysis$TotalI2)
print(calculate_meta(ACC = TRUE)$variance_analysis$Plot)
```
non ntested model
```{r }
print(calculate_meta(ACC=TRUE)$model_uni)
```
nested vs non nested model
```{r }
print(calculate_meta(ACC = TRUE)$model_comparison)

```

# 3.5.1.1 Outliers and influential cases ACC
...
# Example usage:
results <- identify_outliers("ACC", multi = TRUE)
print(results$plot)
...
diagnostic_analysis <- function(data_type, threshold = 2) {
    ...
    if (length(outliers_cooks) > 0) {
        outlier_info <- rbind(outlier_info, data.frame(
            ES_ID = data[outliers_cooks, "ES_ID"],
            PlotLabel = data[outliers_cooks, "PlotLabel"],
            CognitiveFunction = data[outliers_cooks, "CognitiveFunction"],
            Criterion = "Cook's Distance",
            stringsAsFactors = FALSE
        ))
    }

    if (length(outliers_hatvals) > 0) {
        outlier_info <- rbind(outlier_info, data.frame(
            ES_ID = data[outliers_hatvals, "ES_ID"],
            PlotLabel = data[outliers_hatvals, "PlotLabel"],
            CognitiveFunction = data[outliers_hatvals, "CognitiveFunction"],
            Criterion = "Hat Values",
            stringsAsFactors = FALSE
        ))
    }

    if (length(outliers_dfbetas) > 0) {
        outlier_info <- rbind(outlier_info, data.frame(
            ES_ID = data[outliers_dfbetas, "ES_ID"],
            PlotLabel = data[outliers_dfbetas, "PlotLabel"],
            CognitiveFunction = data[outliers_dfbetas, "CognitiveFunction"],
            Criterion = "DFBETAS",
            stringsAsFactors = FALSE
        ))
    }
...
# 3.5.1.3 Model comparison ACC
```{r run calcualte_meta for ACC}
calculate_meta(ACC = TRUE)
calculate_meta(ACC = TRUE)$model_comparison
```
...
```{r forestploter, fig.width = 15, fig.height = 15}
...
forest_plot <- function(RT = FALSE, ACC = FALSE) {
  ...
  forest_data <- data.frame(
    CognitiveFunction = plot_data$CognitiveFunction,
    DoseLabel = plot_data$DoseLabel,
    StudyLabel = plot_data$PlotLabel,
    N = plot_data$N_final,
    Timepoint = plot_data$Timepoint,
    ES = plot_data$yi,
    LowerCI = plot_data$yi - 1.96 * plot_data$sei,
    UpperCI = plot_data$yi + 1.96 * plot_data$sei,
  ...
  overall_N <- forest_data %>%
    distinct(StudyLabel, .keep_all = TRUE) %>%
    summarise(sum(N)) %>%
    pull()
  ...
  # Add the overall Hedges' g row
    	overall_row <- data.frame(
            StudyLabel = "Total (Unique)",
            DoseLabel = "",
            CognitiveFunction = "",
            N = overall_N,
            Timepoint = "",
            ES = model_results$model_multi$beta,
            LowerCI = model_results$model_multi$ci.lb,
            UpperCI = model_results$model_multi$ci.ub,
            ci = sprintf("%.2f (%.2f, %.2f)", model_results$model_multi$beta, model_results$model_multi$ci.lb, model_results$model_multi$ci.ub)
        )
  
  overall_row$` ` <- paste(rep(" ", 20), collapse = " ")
  ...
  p <- forestploter::forest(
    forest_data[,c("CognitiveFunction", "DoseLabel", "StudyLabel", "N", "Timepoint", " ", "ci")],
    est = forest_data$ES,
    ...
    xlim = xlim_values,
    xlab = xlab_text,
    group = forest_data$CognitiveFunction,
    theme = tm
  )
...
# 3.5.1.4.1 Timing as moderator ACC
...
data_ACC$timing <- as.factor(data_ACC$timing)
nrow(data_ACC)
...
# 3.5.1.4.2 Dosage as moderator ACC
```{r Dosage}
...
```
# 3.5.1.4.3 Cognitive function as moderator ACC
...
```
# 3.5.1.4.4 EF sensitivity as moderator ACC
```{r moderation EF sensitivity}
moderation_analysis(ACC = TRUE, moderation_var = "EF_sensitivity")
moderation_analysis(ACC = TRUE, moderation_var = "ID")
table(data_RT$EF_sensitivity)

```

# 3.5.1.4.5 Metaforest Analysis ACC
...
```{r Figure S1 Convergence graph Metaforest ACC} 
#,cache = TRUE
retain_mods <- run_metaforest_analysis("ACC")
conv <- retain_mods$check_conv
plot(conv)


save_plot <- function(conv) {
    file_path <- file.path(getwd(), "figures/supplementary", "Metaforest_ACC_Conv.png")
...
#save_plot(plot(conv))
```
```{r Figure S2 retained moderators ACC}
...
save_plot <- function(preselected) {
    file_path <- file.path(getwd(), "figures/supplementary", "Metaforest_ACC_Preselected.png")
    png(filename = file_path, width = 3500, height = 2200, res = 300)
...
fine_tune_metaforest <- function(data_type, predictors) {
    ...
    data$DoseLabel <- as.factor(data$DoseLabel)
    data$CognitiveFunction <- as.factor(data$CognitiveFunction)
...
```{r Figure S3 Variable/Permutation Importance ACC}
#, cache = TRUE}
predictors <- c("DoseLabel", "CognitiveFunction")

#, "dosage_cat_all", "EF_sensitivity", "CognitiveFunction") # adjust based on previous analysis

mf_cv <- fine_tune_metaforest("ACC", predictors)
# Check the results
print(mf_cv$r2_oob)
print(mf_cv$r2_cv)
final <- mf_cv$final
...
varIm_ACC <- VarImpPlot(final)
VarImpPlot(final)

```

```{r Figure S4 Partial Dependence Plot ACC}
...
```{r Figure S5 Moderated Partial Dependence Plot ACC}
# moderated partial dependence plot
PartialDependence(final, moderator = "CognitiveFunction", rawdata = TRUE, pi = .95)
PartialDependence(final, moderator = "DoseLabel", rawdata = TRUE, pi = .95)
```



# P curve analysis ACC
```{r P curve analysis}
library(ggplot2)
library(zcurve)
library(gridExtra)
...
print(acc_results$p_curve_data)
#print z curve fit results
print(acc_results$z_curve_fit)
...
# }
if (!is.null(acc_results$z_curve_fit)) {
  print(summary(acc_results$z_curve_fit, all = TRUE))
} else {
  print("Insufficient data for Z-curve analysis of accuracy data")
}
